  ...
  Drupal.behaviors.contactPicker = {
    attach: function (context, settings) {
      $('.contact-picker').click(function() {
        var popup = $('<div></div>').attr('id', 'contact-picker-dialog').css('height', 'auto');
        ...
        popup.append($('<iframe></iframe>').attr('id', 'email-scraper').css('width', '500px').css('min-height', '270px').css('border', '0px').css('background', '#fff').attr('src', '/contact-picker/' + $(this).attr('data-nid')));
        // popup!
        popup.dialog({
          resizable: false,
          draggable: false,
