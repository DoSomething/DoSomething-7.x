...
class ConductorActivityClubsSurveyNextResponse extends ConductorActivity {
  ...
  // Message returned to the user if they reject the invite, or send an invalid answer
  ...
  public function run($workflow) {
    ...
    if (self::hasAcceptResponse($_REQUEST['args'])) {
      ...
      $responses = array(
        ...
        'c' => t("Congrats on graduating! You can choose a new club leader for your Club profile page at DoSomething.org/MyClub to pass on the torch."),
  ...
  private function hasAcceptResponse($response) {
    ...
    if (in_array($response, $this->answers)) {
      return TRUE;
    }
