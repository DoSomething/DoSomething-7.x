  ...
  Drupal.behaviors.crazy = {
    ...
    attach: function(context, settings) {
      ...
      if ($('.image-preview img').length > 0) {
        $('.image-preview img').appendTo('.image-widget-data').css({
          'margin-top': '-50px',
          'margin-bottom': '0',
      ...
      else if ($('#edit-submitted-field-cas-image-und-0-upload').length) {
        $('<div></div>').attr('class', 'upload-placeholder').text(Drupal.t('Upload a picture and it will appear here')).css({
          'text-align': 'center',
          'font-size': '30px',
          'font-weight': '600',
          'line-height': '1.2',
      ...
      if ($('input[value="Submit"]').length > 0) {
        $('#edit-submitted-field-cas-top-text-und-0-value, #edit-submitted-field-cas-bottom-text-und-0-value').keydown(function(e) {
          if (e.which == 13) {
            e.stopPropagation();
            return false;
          }
