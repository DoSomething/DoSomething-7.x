   ...
   Drupal.behaviors.dsCrazyScripts = {
	 ...
	 started_page: false,
	    ...
	    if (typeof Drupal.settings.campaign.shares === 'object') {
	    	for (i in Drupal.settings.campaign.shares) {
	    		// Crazy
	    		//if (Drupal.settings.campaign.shares[i].type == 10) {
				//	$('.s-' + Drupal.settings.campaign.shares[i].sid + ' .crazy-button a[rel="' + Drupal.settings.campaign.shares[i].sid + '"]').addClass('clicked');
				//}
				// Vouch
			...
			//	if (Drupal.settings.campaign.shares[i].type == 11) {
			//		$('.s-' + Drupal.settings.campaign.shares[i].sid + ' .vouch-button a[rel="' + Drupal.settings.campaign.shares[i].sid + '"]').addClass('clicked');
			//	}
			//	// BS
			//	else if (Drupal.settings.campaign.shares[i].type == 2) {
			//		$('.s-' + Drupal.settings.campaign.shares[i].sid + ' .bs-button a[rel="' + Drupal.settings.campaign.shares[i].sid + '"]').addClass('clicked');
			//	}
	    	}
	    }
	 ...
	 fb_refresh: function(callback) {
	 	if (!Drupal.behaviors.dsCrazyScripts.started_page) {
		 	...
		 	window.fbAsyncInit = function() {
			    ...
			    if (Drupal.settings.campaign.origin == 2 || Drupal.settings.campaign.origin == 3) {
	 ...
	 fb_status: function() {
		 Drupal.behaviors.fb.gate({
		   gate_call_fb: 2,
		   gate_app_id: Drupal.settings.campaign.fb_app_id
		 }, function(response) {  });
	 },
  };
